class SessionsController < ApplicationController
  ...
  def create
    ...
    # Is the user logging in to obtain an oAuth token?
    if session[:oauthparams]
      ...
      path = '/#/auths' + '?rtype=redirect&' + rtype + '&' + ruri + '&' + cid
      tempuser = session[:user_id]
      reset_session
      session[:user_id] = tempuser
    else
      path = root_path + '#/auths'
    end
    ...
    if signed_in?
      ...
      if @authentication.user_id.present?
        ...
        self.current_user = u
        if oauth
          redirect_to '/#/cards?' + rtype + '&' + ruri + '&' + cid + ident  #root_path, notice: "Welcome to The app!", :anchor => nil
        else
          redirect_to path
        end
      end
